...
function encodeToRot13(str) {
  let charCodeArray = [...str].map((letter) => letter.charCodeAt(0));
  charCodeArray = charCodeArray.map((char) => {
    let result = char;
    if ((char >= 97 && char <= 109) || (char >= 65 && char <= 77)) {
      result += 13;
    } else if ((char >= 110 && char <= 122) || (char >= 66 && char <= 90)) {
      result -= 13;
    }
    return result;
  });
  charCodeArray = charCodeArray.map((char) => String.fromCharCode(char));
